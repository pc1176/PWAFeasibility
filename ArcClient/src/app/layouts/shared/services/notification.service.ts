...
export class NotificationService {
  private readonly VAPID_PUBLIC_KEY = 'BN3Kb63jhRZovqI1r3VctC7avbST-0x87HlS19gNzg1aK7fReU2yyNHaqBJYwaJ-DtozjM9Tf2JqcmOib9O0C3A';
  private readonly API_URL = 'https://league-picks-shot-guarantees.trycloudflare.com/api/notifications';
  ...
  public async sendTestNotification(msg: string) {
    ...
    try {
      const response = await firstValueFrom(
        this.http.post(
          ...
          msg,
          {
            headers: { 'Content-Type': 'text/plain' }  // ✅ Explicitly set Content-Type as text
          }
        ).pipe(
          timeout(5000),
          retry(3)
